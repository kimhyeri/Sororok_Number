...
class MemberSearchView: UIView {
    ...
    override func awakeFromNib()
    {
        ...
        self.searchBar.translatesAutoresizingMaskIntoConstraints = false
        self.searchBar.placeholder = "누굴 찾으시나요?"
...
extension MemberSearchView : UISearchBarDelegate {
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        guard searchBar.text != nil else { return }
        let searchMember = Notification.Name(rawValue: searchMemberNotificationKey)
         NotificationCenter.default.post(name: searchMember, object: nil, userInfo: ["text":searchBar.text])
    }
    
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        if searchBar.text?.count == 0 {
            let searchMemberDone = Notification.Name(rawValue: searchMemberDoneNotificationKey)
            NotificationCenter.default.post(name: searchMemberDone, object: nil)
        }
    }
}
